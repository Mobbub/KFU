...
def load_dataset():
    with open("dataset_10_px.pkl", "rb") as f:
...
def train_predict_task_2():
    images, labels = load_dataset()

    weights_input_to_hidden = np.random.uniform(-0.5, 0.5, (30, 100))
    weights_hidden_to_output = np.random.uniform(-0.5, 0.5, (5, 30))
    ...
    test_image = plt.imread("pic/resized_image_10.jpg", format="jpeg")

    gray = lambda rgb: np.dot(rgb[..., :3], [0.299, 0.587, 0.114])
    test_image = 1 - (gray(test_image).astype("float32") / 255)

    test_image = np.resize(test_image, (10, 10)).reshape(-1, 1)
